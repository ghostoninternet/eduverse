      ...
      async function fetchExercises() {
        try {
          const details = await Promise.all(
            moduleDetail.exercises.map(async (exercise) => {
              const exerciseDetail = await getExerciseDetail(exercise.id);
              console.log("Exercise detail fetched ui:", exerciseDetail); // Log kiểm tra
              return exerciseDetail; // Lấy toàn bộ dữ liệu
            })
          );
          setExercisesDetail(details);
          console.log("All Exercise details:", details); // Log tổng hợp
        } catch (error) {
    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center">
      ...
      >
        ...
        <div className="mb-4">
          ...
          <p><strong>Title:</strong> {moduleDetail.title || "N/A"}</p>
          <p><strong>Description:</strong> {moduleDetail.description || "No description available"}</p>
          <p><strong>Course:</strong> {moduleDetail.courseTitle || "Unknown"}</p>
          <p><strong>Created At:</strong> {new Date(moduleDetail.createdAt).toLocaleDateString()}</p>
        </div>

        {/* Video Lessons */}
        <div className="mb-4">
          <h3 className="text-lg font-bold">Video Lessons</h3>
          {Array.isArray(moduleDetail.videoLessons) && moduleDetail.videoLessons.length > 0 ? (
            moduleDetail.videoLessons.map((video, index) => (
              <div key={index} className="bg-gray-100 p-2 my-2 rounded-lg">
                ...
                <p><strong>Duration:</strong> {video.videoLength} minutes</p>
                {/* Video Player */}
        ...
        <div className="mb-4">
          ...
          {Array.isArray(exercisesDetail) && exercisesDetail.length > 0 ? (
            exercisesDetail.map((exerciseDetail, index) => {
              const exercise = exerciseDetail.data;
              return (
                <div key={index} className="bg-gray-100 p-4 my-2 rounded-lg">
                  <h4 className="font-bold">{exercise?.name || `Exercise ${index + 1}`}</h4>
                  <p><strong>Duration:</strong> {exercise?.duration || "N/A"} minutes</p>
                  <p><strong>Pass Score:</strong> {exercise?.passScore || "N/A"}</p>
                  {Array.isArray(exercise?.quizzes) && exercise.quizzes.length > 0 ? (
                    <div>
                      <h5 className="font-bold mt-2">Quizzes:</h5>
                      {exercise.quizzes.map((quiz, quizIndex) => (

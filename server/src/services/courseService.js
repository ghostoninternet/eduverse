...
const getCourseDetail = async (courseId) => {
  ...
  foundCourse = excludeObjectKeys(foundCourse, ["updatedAt"]);
  foundCourse.courseInstructor = instructorInfo;
  // Get all category name
  let categoryNames = [];
  for (let i = 0; i < foundCourse.courseCategory.length; i++) {
    let foundCategoryName = await categoryDaos.findCategoryById(
      foundCourse.courseCategory[i]
    );
  ...
  for (let i = 0; i < foundCourse.courseModules.length; i++) {
    let foundModuleInfo = await moduleDaos.findModuleById(
      foundCourse.courseModules[i]
    );
    if (foundModuleInfo) {
      // Get module exercise info
      let exerciseInfo = [];
      for (let j = 0; j < foundModuleInfo.moduleExercises.length; j++) {
        let foundExerciseInfo = await exerciseDaos.findExerciseById(
          foundModuleInfo.moduleExercises[j]
        );
...
const searchCourses = async (queryParams, limit, page) => {
  ...
  if (courseCategory) {
    ...
    for (let i = 0; i < courseCategory.length; i++) {
      const foundCategory = await categoryDaos.findCategoryByName(
        courseCategory[i]
      );
...
const getInstructorCourses = async (instructorId, limit = 10, page = 1) => {
  // Find all courses
  let foundCourses = await courseDaos.findCourses(
  ...
  for (let i = 0; i < foundCourses.length; i++) {
    ...
    for (let j = 0; j < foundCourses[i].courseCategory.length; j++) {
      let foundCategory = await categoryDaos.findCategoryById(
        foundCourses[i].courseCategory[j]
      );
      if (foundCategory) {
...
const getInstructorCourseDetail = async (courseId) => {
  ...
  for (let i = 0; i < transformedCourse.courseCategory.length; i++) {
    const foundCategory = await categoryDaos.findCategoryById(
      transformedCourse.courseCategory[i]
    );
    if (foundCategory) {
      categories.push(foundCategory.categoryName);
    }
  ...
  for (let i = 0; i < transformedCourse.courseModules.length; i++) {
    let foundModule = await moduleDaos.findModuleById(
      transformedCourse.courseModules[i]
    );
    if (foundModule) {
      let moduleExercises = [];
      for (let j = 0; j < foundModule.moduleExercises.length; j++) {
        let foundExercise = await exerciseDaos.findExerciseById(
          foundModule.moduleExercises[j]
        );
        if (foundExercise) {
          moduleExercises.push(
            includeObjectKeys(foundExercise, [
              "_id",
              "exerciseName",
              "exerciseDuration",
            ])
...
const createNewCourse = async (instructorId, newCourseData) => {
  ...
  if (foundCourse)
    throw new CustomError.BadRequestError(
      "There's already a course with the same name"
    );
  ...
  for (let i = 0; i < newCourseData.courseCategory.length; i++) {
    let foundCategory = await categoryDaos.findCategoryById(
      newCourseData.courseCategory[i]
    );
    if (!foundCategory)
      throw new CustomError.NotFoundError("No category found!");
...
const updateCourse = async (courseId, updateCourseData) => {
  ...
  for (let i = 0; i < updateCourseData.courseCategory.length; i++) {
    let foundCategory = await categoryDaos.findCategoryById(
      updateCourseData.courseCategory[i]
    );
    if (!foundCategory)
      throw new CustomError.NotFoundError("No category found!");
    categories.push(foundCategory._id);
  ...
  const updatedCourse = await courseDaos.updateCourse(
    courseId,
  ...
  for (let i = 0; i < updatedCourse.courseCategory.length; i++) {
    let foundCategory = await categoryDaos.findCategoryById(
      updatedCourse.courseCategory[i]
    );
    if (!foundCategory)
      throw new CustomError.NotFoundError("No category found!");
...
const deleteCourse = async (courseId) => {
  ...
  if (foundCourse.isDeleted)
    throw new CustomError.BadRequestError("Course has already been deleted!");

  const deletedCourse = await courseDaos.updateCourse(courseId, {
    isDeleted: true,
  });
  for (let i = 0; i < deletedCourse.courseModules.length; i++) {
    const deletedModule = await moduleDaos.updateModule(
      deletedCourse.courseModules[i],
      { isDeleted: true }
    );
    if (deletedModule) {
      for (let j = 0; j < deletedModule.moduleExercises.length; j++) {
        await exerciseDaos.updateExercise(deletedModule.moduleExercises[j], {
          isDeleted: true,
        });
      }
    }
...
const getAllCourses = async (queryParams, limit, page) => {
  let { categoryName } = queryParams;
  let filter = {};
  if (categoryName) {
    ...
    for (let i = 0; i < categoryName.length; i++) {
      const foundCategory = await categoryDaos.findByName(
        categoryName[i]
      );
      if (foundCategory) categoryIds.push(foundCategory._id);
    }
    filter = {
      courseCategory: { $all: categoryIds },
    };
  }
